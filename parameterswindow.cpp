...
void
ParametersWindow::printResults(FunctionMinimum& min) {
    pMsgWindow->addLine("minimum:\n");
    if(min.IsValid()) {
        pMsgWindow->addLine("Minuit did succesfully converge.\n");
    }
    else {
        pMsgWindow->addLine("WARNING: Minuit did not converge.\n");
    }
    ...
    for(unsigned int i=0; i<min.Parameters().Vec().size(); i++) {
        pMsgWindow->addLine(QString("%1\n"). arg(min.Parameters().Vec()(i), 18));
    }
    if(min.HasValidCovariance()) {
        pMsgWindow->addLine(QString("minimum internal covariance matrix: LASymMatrix parameters:\n"));
        for(unsigned int i=0; i<min.Error().Matrix().Nrow(); i++) {
            for(unsigned int j=0; j<min.Error().Matrix().Ncol(); j++) {
                pMsgWindow->addLine(QString("%1"). arg(min.Error().Matrix()(i,j), 18));
            }
            pMsgWindow->addLine("\n");
        }
    }
    if(!min.IsValid()) {
        pMsgWindow->addLine("WARNING: FunctionMinimum is invalid:\n");
        if(!min.State().IsValid())
           pMsgWindow->addLine("\t State is invalid\n");
        if(min.IsAboveMaxEdm())
           pMsgWindow->addLine("\t Edm is above max");
        if (min.HasReachedCallLimit())
           pMsgWindow->addLine("\t Reached call limit");
    }
}
